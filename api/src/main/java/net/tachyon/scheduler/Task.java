...
public sealed interface Task permits TaskImpl {
    ...
    @NotNull ExecutionType getExecutionType();

    @NotNull Scheduler getOwner();
    ...
    final class Builder {
        ...
        public @NotNull Builder delay(long time, @NotNull TimeUnit unit) {
            return delay(Duration.ofMillis(unit.toMillis(time)));
        }

        public @NotNull Builder delay(long time, @NotNull net.tachyon.utils.time.TimeUnit unit) {
            return delay(Duration.ofMillis(unit.toMilliseconds(time)));
        }
