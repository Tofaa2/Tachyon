...
public class EntityFinder {
    ...
    public List<Entity> find(@Nullable World instance, @Nullable Entity self) {
        if (targetSelector == TargetSelector.MINESTOM_USERNAME) {
            ...
            final Entity entity = Tachyon.getServer().getEntity(constantUuid);
        ...
        if (!gameModes.isEmpty()) {
            result = result.stream().filter(entity -> {
                if (!(entity instanceof Player player))
                    return false;
                return filterToggleableMap(entity, (player).getGameMode(), gameModes);
            }).collect(Collectors.toList());
        }
     ...
     */
    @Nullable
    public Player findFirstPlayer(@Nullable World instance, @Nullable Entity self) {
