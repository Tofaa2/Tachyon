...
public class SerializableDataImpl extends SerializableData {
    ...
    protected static final DataManager DATA_MANAGER = Tachyon.getServer().getDataManager();
     ...
     * <p>
     * WARNING: the type needs to be registered in {@link TachyonDataManager}.
     *
     * @param key   the key
     * @param value the value object
     * @param type  the value type
     * @param <T>   the value generic
     * @throws UnsupportedOperationException if {@code type} is not registered in {@link TachyonDataManager}
     */
    @Override
    public <T> void set(@NotNull String key, @Nullable T value, @Nullable Class<T> type) {
        if (type != null && DATA_MANAGER.getDataType(type) == null) {
            throw new UnsupportedOperationException("Type " + type.getName() + " hasn't been registered in TachyonDataManager#registerType");
        }
    ...
    public byte[] getSerializedData(@NotNull Object2ShortMap<String> typeToIndexMap, boolean indexed) {
        ...
        TachyonBinaryWriter binaryWriter = new TachyonBinaryWriter();
        for (Map.Entry<String, Object> entry : data.entrySet()) {
            ...
            Check.notNull(dataType, "Tried to encode a type not registered in TachyonDataManager: " + type);
        ...
        if (indexed) {
            ...
            TachyonBinaryWriter indexWriter = new TachyonBinaryWriter();
