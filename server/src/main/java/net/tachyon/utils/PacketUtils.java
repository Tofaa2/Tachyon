...
public final class PacketUtils {
    ...
    private static final PacketListenerManager PACKET_LISTENER_MANAGER = (PacketListenerManager) Tachyon.getServer().getPacketListenerManager();
    ...
    public static void sendGroupedPacket(@NotNull Collection<Player> players, @NotNull ServerPacket packet,
        ...
        if (Tachyon.getServer().isPacketGroupingEnabled()) {
            // Send grouped packet...
    ...
    private static ByteBuf getPacketBuffer(@NotNull ServerPacket packet) {
        TachyonBinaryWriter writer;
        if (packet.getId() == ServerPacketIdentifier.CHUNK_DATA) {
            writer = new TachyonBinaryWriter(BufUtils.getBuffer(true, 40_000));
        } else {
            writer = new TachyonBinaryWriter(BufUtils.getBuffer(true));
        }
     ...
     * it depends of it the client did receive a {@link SetCompressionPacket} packet before.
     *
    ...
    public static void compressBuffer(@NotNull Deflater deflater, @Nullable byte[] buffer,
        ...
        if (packetLength < Tachyon.getServer().getCompressionThreshold()) {
    ...
    public static ByteBuf createFramedPacket(@NotNull ServerPacket serverPacket, boolean directBuffer) {
        ...
        if (Tachyon.getServer().getCompressionThreshold() > 0) {
