 ...
 */
public class TachyonBinaryWriter extends OutputStream implements BinaryWriter {

    private ByteBuf buffer;
    private final NBTWriter nbtWriter = new NBTWriter(this, false);

    /**
     * Creates a {@link TachyonBinaryWriter} using a heap buffer with a custom initial capacity.
     *
     * @param initialCapacity the initial capacity of the binary writer
     */
    public TachyonBinaryWriter(int initialCapacity) {
        this.buffer = Unpooled.buffer(initialCapacity);
    }

    /**
     * Creates a {@link TachyonBinaryWriter} from multiple a single buffer.
     *
     * @param buffer the writer buffer
     */
    public TachyonBinaryWriter(@NotNull ByteBuf buffer) {
        this.buffer = buffer;
    }

    /**
     * Creates a {@link TachyonBinaryWriter} from multiple buffers.
     *
     * @param buffers the buffers making this
     */
    public TachyonBinaryWriter(@NotNull ByteBuf... buffers) {
        this.buffer = Unpooled.wrappedBuffer(buffers);
    }

    /**
     * Creates a {@link TachyonBinaryWriter} with a "reasonably small initial capacity".
     */
    public TachyonBinaryWriter() {
    ...
    public void writePoint(@NotNull Point blockPosition) {
        writePoint(blockPosition.blockX(), blockPosition.blockY(), blockPosition.blockZ());
    }

    public void writePoint(int x, int y, int z) {
     ...
     *
     * @return the byte array containing all the {@link TachyonBinaryWriter} data
     */
    ...
    /**
     * Adds a {@link TachyonBinaryWriter}'s {@link ByteBuf} at the beginning of this writer.
     *
     * @param headerWriter the {@link TachyonBinaryWriter} to add at the beginning
     */
    public void writeAtStart(@NotNull BinaryWriter headerWriter) {
        // Get the buffer of the header
        final ByteBuf headerBuf = ((TachyonBinaryWriter) headerWriter).getBuffer();
    ...
    /**
     * Adds a {@link TachyonBinaryWriter}'s {@link ByteBuf} at the end of this writer.
     *
     * @param footerWriter the {@link TachyonBinaryWriter} to add at the end
     */
    public void writeAtEnd(@NotNull BinaryWriter footerWriter) {
        // Get the buffer of the footer
        final ByteBuf footerBuf = ((TachyonBinaryWriter) footerWriter).getBuffer();
