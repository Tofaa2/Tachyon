...
public class EntityFinder {
    ...
    // Position
    private Position startPosition = Position.ZERO;
    ...
    public List<Entity> find(@Nullable World instance, @Nullable Entity self) {
        if (targetSelector == TargetSelector.MINESTOM_USERNAME) {
            ...
            final TachyonEntity entity = TachyonEntity.getEntity(constantUuid);
        ...
        if (!entityTypes.isEmpty()) {
            result = result.stream().filter(entity ->
                    filterToggleableMap(entity, ((TachyonEntity)entity).getEntityType(), entityTypes))
                    .collect(Collectors.toList());
        }
        ...
        if (!gameModes.isEmpty()) {
            result = result.stream().filter(entity -> {
                if (!(entity instanceof TachyonPlayer))
                    return false;
                return filterToggleableMap(entity, ((TachyonPlayer) entity).getGameMode(), gameModes);
            }).collect(Collectors.toList());
        }
        ...
        if (level != null) {
            ...
            result = result.stream().filter(entity -> {
                ...
                final int level = ((TachyonPlayer) entity).getLevel();
        ...
        if (!names.isEmpty()) {
            result = result.stream().filter(entity -> {
                if (!(entity instanceof TachyonPlayer))
                    return false;
                return filterToggleableMap(entity, ((TachyonPlayer) entity).getUsername(), names);
            }).collect(Collectors.toList());
        }
                    ...
                    .sorted((ent1, ent2) -> switch (entitySort) {
                        case ARBITRARY, RANDOM ->
                            // RANDOM is handled below
                                1;
                        case FURTHEST -> startPosition.distance(ent1.getPosition()) >
                                startPosition.distance(ent2.getPosition()) ?
                                1 : 0;
                        case NEAREST -> startPosition.distance(ent1.getPosition()) <
                                startPosition.distance(ent2.getPosition()) ?
                                1 : 0;
                    })
    ...
    public List<Entity> find(@NotNull CommandSender sender) {
        if (sender.isPlayer()) {
            TachyonPlayer player =(TachyonPlayer) sender.asPlayer();
     ...
     */
    @Nullable
    public Player findFirstPlayer(@Nullable Instance instance, @Nullable TachyonEntity self) {
        final List<Entity> entities = find(instance, self);
        for (Entity entity : entities) {
            if (entity instanceof TachyonPlayer) {
                return (TachyonPlayer) entity;
            }
        }
        return null;
    }
    ...
    public Player findFirstPlayer(@NotNull CommandSender sender) {
        if (sender.isPlayer()) {
            final TachyonPlayer player =(TachyonPlayer) sender.asPlayer();
    ...
    @Nullable
    public Entity findFirstEntity(@Nullable World instance, @Nullable Entity self) {
        final List<Entity> entities = find(instance, self);
        for (Entity entity : entities) {
            return entity;
        }
    ...
    @NotNull
    private static List<Entity> findTarget(@Nullable World instance, @NotNull TargetSelector targetSelector,
