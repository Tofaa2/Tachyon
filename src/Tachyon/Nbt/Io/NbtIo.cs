    ...
    NbtIo<TIn, TOut>.INameWriter<TOut> nameWriter)
{
    ...
    protected readonly Dictionary<NbtType, Func<NbtLimiter, TIn, Nbt>> TagReaders = new();
    protected readonly Dictionary<NbtType, Action<TOut, Nbt>> TagWriters = new();

    
    public void RegisterType<T>(NbtType type, Func<NbtLimiter, TIn, T> reader, Action<TOut, T> writer) where T : Nbt
    {
        TagReaders[type] = reader;
        TagWriters[type] = (to, tag) => writer(to, (T) tag);
    }

    public Nbt? DeserializeNbt(NbtLimiter limiter, TIn from, bool named = true)
    {
    ...
    public void SerializeNbt(TOut to, Nbt tag, bool named = true)
    {
        var type = tag.Type;
    ...
    public void WriteNbt(TOut stream, Nbt tag)
    {
    ...
    public Nbt ReadNbt(NbtLimiter limiter, TIn stream, NbtType type)
    {
